...
=======
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body">
            <ul class="nav nav-tabs card-header-tabs">
                <li class="nav-item">
                    <a class="nav-link {{ $activeTab === 'items' ? 'active' : '' }}" href="{{ route('admin.inventory.index', ['tab' => 'items']) }}">Items</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ $activeTab === 'store-stock' ? 'active' : '' }}" href="{{ route('admin.inventory.index', ['tab' => 'store-stock']) }}">Store Stock</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ $activeTab === 'vendor-return' ? 'active' : '' }}" href="{{ route('admin.inventory.index', ['tab' => 'vendor-return']) }}">Vendor Return</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ $activeTab === 'stock-ledger' ? 'active' : '' }}" href="{{ route('admin.inventory.index', ['tab' => 'stock-ledger']) }}">Stock Ledger</a>
                </li>
            </ul>
        </div>
    </div>
    ...
    @elseif($activeTab === 'store-stock')
        <div class="row g-3">
            <div class="col-12">
                <div class="card shadow-sm">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Store Stock</h5>
                        <span class="badge bg-secondary">{{ count($balances ?? []) }} items</span>
                    </div>
                    <div class="card-body table-responsive">
                        <table class="table table-sm table-striped align-middle">
                            <thead>
                            ...
                            <tbody>
                                @forelse($balances ?? [] as $row)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->name ?? '—' }}</td>
                                        <td>{{ $item->category ?? 'Uncategorized' }}</td>
                                        <td>{{ number_format((float) ($row['balance'] ?? 0), 3) }} {{ $item->uom ?? '' }}</td>
                                        <td>{{ number_format((float) ($item->reorder_level ?? 0), 3) }} {{ $item->uom ?? '' }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted">No store stock found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    @elseif($activeTab === 'vendor-return')
        <div class="card shadow-sm">
            <div class="card-header">
                <h5 class="mb-0">Vendor Return</h5>
            </div>
            <div class="card-body text-muted">
                Vendor Return tab is available. Existing workflows remain untouched in this task.
            </div>
        </div>
    @elseif($activeTab === 'stock-ledger')
        <div class="card shadow-sm mb-3">
            <div class="card-header">
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('admin.inventory.index') }}" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-5">
                        ...
                        <input type="text" name="search" class="form-control" value="{{ request('search') }}" placeholder="Item code, item name, category, type, reference, remarks">
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a class="btn btn-outline-secondary w-100" href="{{ route('admin.inventory.index', ['tab' => 'stock-ledger']) }}">Reset</a>
                    </div>
                </form>
            </div>
>>>>>>> Stashed changes
                ...
                <table class="table table-sm table-striped align-middle">
                    ...
                    <tbody>
                        @forelse($stockLedger as $row)
                            <tr>
                                <td>{{ $row->txn_at ? \Illuminate\Support\Carbon::parse($row->txn_at)->format('Y-m-d H:i') : '—' }}</td>
                                <td>{{ $row->item_sku ?? '—' }}</td>
                                <td>{{ $row->item_name ?? '—' }}</td>
                                <td>{{ $row->txn_type ?? '—' }}</td>
                                <td>{{ number_format((float) ($row->trans_quantity ?? $row->quantity ?? 0), 3) }}</td>
                                <td>{{ $row->trans_unit_code ?: ($row->item_uom ?? '—') }}</td>
                                <td>{{ number_format((float) ($row->unit_cost ?? 0), 2) }}</td>
                                <td>
                                    @if($row->reference_type || $row->reference_id)
                                        {{ $row->reference_type ?? 'REF' }}{{ $row->reference_id ? ' #'.$row->reference_id : '' }}
                                    @else
                                        —
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted">No stock ledger transactions found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            @if(method_exists($stockLedger, 'links'))
